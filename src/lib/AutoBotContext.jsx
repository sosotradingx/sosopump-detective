...
// Flag-uri la nivel de modul - previn suprapunerea ciclurilor de scan și monitor
let paperBotRunningGlobal = false;
let exitMonitorRunning = false;
...
// astfel încât atât botul cât și monitorul de SL/TP să ruleze independent de
// pagina afișată. SL/TP/partial-TP/low-score se verifică aici (sursă unică),
// eliminând dublarea cu logica din PaperTrading.jsx.
export function AutoBotProvider({ children }) {
  ...
  const log = useCallback((msg) => setBotLog(prev => [`[${new Date().toLocaleTimeString("ro-RO")}] ${msg}`, ...prev.slice(0, 29)]), []);

  // === MONITOR DE IEȘIRE (15s) - sursă unică pentru SL/TP/partial-TP/low-score ===
  // Rulează mereu, indiferent de autoEnabled, ca să protejeze și pozițiile manuale.
  const checkExits = useCallback(async () => {
    if (exitMonitorRunning) return;
    exitMonitorRunning = true;
    try {
      ...
      for (const trade of openTrades) {
        ...
        // Partial TP (doar dacă nu ies deja pe SL/TP)
        if (!reason && cfg.usePartialTP && !trade.partial_tp_hit) {
          ...
          if (cur >= tp1Price && trade.quantity > 0) {
            ...
            await base44.entities.PaperTrade.create({
              symbol: trade.symbol, side: "BUY", status: "closed",
              entry_price: trade.entry_price, exit_price: cur, quantity: partialQty,
            ...
            await base44.entities.PaperTrade.update(trade.id, {
              quantity: remainQty, partial_tp_hit: true, stop_loss: newSL,
              notes: `${trade.notes || ""} | TP1 executat la ${cur.toFixed(4)}`,
            });
            log(`🎯 PARTIAL TP ${trade.symbol} | Vândut ${cfg.partialTPPercent ?? 50}% la +${pnlPct.toFixed(2)}% | Rămâne: ${remainQty}${cfg.moveSlToBreakeven ? " | SL→Breakeven" : ""}`);
            anyChanged = true;
            continue;
          }
        }

        // Low-score exit opțional
        ...
        if (reason) {
          ...
          log(`${reason === "stop_loss" ? "🔴 SL HIT" : "🟢 TP HIT"} ${trade.symbol} @ $${cur} | ${pnlPct >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%`);
          anyChanged = true;
        }
      }
      if (anyChanged) queryClient.invalidateQueries({ queryKey: ["paper-trades"] });
    } catch (err) {
      console.warn("Exit monitor skipped a cycle:", err?.message || err);
    } finally {
      exitMonitorRunning = false;
    }
  }, [queryClient, log]);
  ...
  const runAutoBot = useCallback(async (sessionId) => {
    ...
    if (sessionId && !isActiveBotSession(sessionId)) return;
    ...
    try {
      ...
      freshClosed.forEach(t => {
        ...
        if (!lastClosedMap[t.symbol] || closedAt > lastClosedMap[t.symbol]) lastClosedMap[t.symbol] = closedAt;
      });
      const candidates = pairs
        .filter(p => !openSymbols.has(p.symbol))
        .filter(p => !(lastClosedMap[p.symbol] && (now - lastClosedMap[p.symbol]) < cooldownMs))
        .slice(0, Math.min(cfg.scanPairs ?? 100, pairs.length));

      let opened = 0, scanned = 0, topScore = 0, topSymbol = "";
  ...
  // ===== Cîrță sesiuni orfane: marchează ca "stopped" sesiunile vechi ale userului =====
  // (heartbeat > 90s sau oricare "running" înainte de a crea una nouă).
  const cleanupOrphanSessions = useCallback(async (currentSessionId) => {
    try {
      const me = await base44.auth.me().catch(() => null);
      if (!me) return;
      const allSessions = await base44.entities.BotSession.filter({ created_by: me.email, status: "running" }, "-created_date", 50);
      const cutoff = Date.now() - 90 * 1000;
      const orphans = allSessions.filter(s => {
        if (s.session_id === currentSessionId) return false;
        const hb = s.last_heartbeat ? new Date(s.last_heartbeat).getTime() : new Date(s.started_at || s.created_date).getTime();
        return hb < cutoff;
      });
      await Promise.all(orphans.map(s => base44.entities.BotSession.update(s.id, { status: "stopped" }).catch(() => {})));
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    cleanupOrphanSessions(sessionId);
  ...
  }, [autoEnabled, cleanupOrphanSessions]);
